    ...
    private static void generateDistanceFieldSimple(BitmapRef<float[]> output,
        ...
        for (int y = 0; y < output.height; ++y) {
            ...
            for (int col = 0; col < output.width; ++col) {
                ...
                output.pixels[pixelIndex] = (float) distanceMapping.map(distance.r);
                output.pixels[pixelIndex + 1] = (float) distanceMapping.map(distance.g);
                output.pixels[pixelIndex + 2] = (float) distanceMapping.map(distance.b);
    ...
    private static void generateDistanceFieldOverlapping(BitmapRef<float[]> output,
        ...
        for (int y = 0; y < output.height; ++y) {
            ...
            for (int col = 0; col < output.width; ++col) {
                ...
                // Convert distance to pixel values and write to output
                int pixelIndex = output.sectionOperator(x, y);
                output.pixels[pixelIndex + 0] = (float) distanceMapping.map(distance.r);
                output.pixels[pixelIndex + 1] = (float) distanceMapping.map(distance.g);
                output.pixels[pixelIndex + 2] = (float) distanceMapping.map(distance.b);
